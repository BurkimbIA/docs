# documentation

- [home](index.md)

## how-to guides

- [data access](how-to/data_storage.md)
- [API inferences](how-to/inference.md)

## API

- [Cookbook API (recettes curl & Python)](api/cookbook.md)

## explanations

- [machine translation stack](explanations/machine_translation.md)
- [plan SOTA pour un LLM Moore](explanations/moore_llm_sota_plan.md)
- [deploiement llm sans cluster](explanations/deploiement_llm_serverless.md)

## tutorials

- [runpod serverless llm tutoriel](tutorials/runpod_serverless.md)
- [runpod pods setup](tutorials/runpod_pods_setup.md)
- [runpod mcp launcher llm training](tutorials/runpod_mcp_launcher.md)

## references

- [documentation style](references/index.md)
